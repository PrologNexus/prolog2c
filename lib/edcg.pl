...
%%% dynamic declaration of accumulators and predicates

pass_info(X) :- recordz(edcg_pass_info, pass_info(X)).
pass_info(X, Y) :- recordz(edcg_pass_info, pass_info(X, Y)).

acc_info(X, Y, Z, U, V) :- recordz(edcg_acc_info, acc_info(X, Y, Z, U, V)).
acc_info(X, Y, Z, U, V, I, J) :- recordz(edcg_acc_info, pass_info(X, Y, Z, U, V, I, J)).

pred_info(X, Y, Z) :- recordz(edcg_pred_info, pred_info(X, Y, Z)).
