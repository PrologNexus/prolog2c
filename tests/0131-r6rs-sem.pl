...
reduce(P1, Next) :-
  ...
  append(Es, [E|Es2], Es_E_Es), e_s(Es), e_s(Es2), e(E), \+(v(E)),
  exist_e(Es, Es2),
...
reduce(P1, Next) :-  % 61arity
  ctx_p(P1, [Proc1|Vs], Next,
        [raise, ['make-cond','arity mismatch']], normal),
  proc1(Proc1), v_s(Vs), length(Vs, Len), \+(Len == 1).
reduce(P1, Next) :-  %62arity
  ctx_p(P1, [Proc2|Vs], Next,
        [raise, ['make-cond','arity mismatch']], normal),
  proc2(Proc2), v_s(Vs), length(Vs, Len), \+(Len == 2).
...
reduce(PG1, Next) :-  % 6weherr
  ctx_pg(PG1, ['with-exception-handler', V1, V2],
         ...
         normal),
  v(V1), v(V2), or(V1, V2, V12), \+(proc(V12)).
...
eval(X, Y) :- reduce(X, Z), \+(X==Z), eval(Z, Y), !.
eval(X, X) :- reduce(X, X), !.
...
bir_v(X, [lambda, Xs, E|Es]) :-
  x_s(Xs), e(E), e_s(Es),
  \+(member(X, Xs)), bir_v(X, [begin, E|Es]), !.
bir_v(X, [lambda, Xs_dot_X2, E|Es]) :-
  append(Xs, ['.'|[X2]], Xs_dot_X2), x_s(Xs), x(X2),
  e(E), e_s(Es),
  \+(member(X, Xs)), \+(X = X2), bir_v(X, [begin, E|Es]), !.
bir_v(X, [lambda, X2, E|Es]) :-
  x(X2), e(E), e_s(Es), \+(X = X2), bir_v(X, [begin, E|Es]), !.
bir_v(X, [letrec, Binds, E|Es]) :-
  no_quote_bind_s(Binds), e(E), e_s(Es),
  binds_split(Binds, Vars, Exps),
  \+(member(X, Vars)), append(Exps, [E|Es], Z),
  bir_v(X, [begin|Z]), !.
bir_v(X, ['letrec*', Binds, E|Es]) :-
  no_quote_bind_s(Binds), e(E), e_s(Es),
  binds_split(Binds, Vars, Exps),
