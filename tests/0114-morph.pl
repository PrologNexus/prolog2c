...
tokenize_stream(Tokens) :-
   tokenize_line_dl(Tokens/Tail),
   tokenize_stream(Tail).
...
tokenize_line_dl(Dlist) :-
   get_char_and_type(Char,Type),
   tokenize_line_x(Type,Char,Dlist).
...
get_char_and_type(Char,Type) :-
   get0(C), atom_codes(C1, [C]),
   char_type_char(C1,Type,Char).


% tokenize_line_x(+Type,+Char,-Tokens/Tail)
...
tokenize_line_x(whitespace,_,Dlist) :-       % whitespace, skip it
   !,
   tokenize_line_dl(Dlist).
...
tokenize_line_x(letter,Char,[w(T)|Tokens]/Tail) :-
   !,
   tokenize_letters(letter,Char,T,NewType,NewChar),
   tokenize_line_x(NewType,NewChar,Tokens/Tail).

tokenize_line_x(digit,Char,[n(T)|Tokens]/Tail) :-
   !,
   tokenize_digits(digit,Char,T,NewType,NewChar),
   tokenize_line_x(NewType,NewChar,Tokens/Tail).
...
tokenize_line_x(special,Char,[s(Char)|Tokens]/Tail) :-   % special char
   !,
   tokenize_line_dl(Tokens/Tail).

tokenize_line_x(_,Char,[other(Char)|Tokens]/Tail) :-     % unidentified char
   !,
   tokenize_line_dl(Tokens/Tail).



% tokenize_letters(+Type,+Char,-Token,-NewChar,-NewType)
...
tokenize_letters(letter,Char,[Char|Rest],NewType,NewChar) :-
   % It's a letter, so process it, read another character ahead, and recurse.
   !,
   get_char_and_type(Char2,Type2),
   tokenize_letters(Type2,Char2,Rest,NewType,NewChar).

tokenize_letters(_,'''',Rest,NewType,NewChar) :-
   %
...
tokenize_digits(Type,Char,[],Type,Char).
